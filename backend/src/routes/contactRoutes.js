...
const normalizePhone = (phoneNo) => {
  try {
    ...
    console.log("Parsed phone number:", parsed); // Default region: India
    return parsed.number; // Returns E.164 format like +919876543210
  } catch {
...
router.post('/sync-contacts', auth, async (req, res) => {
  try {
    ...
    // Extract phone numbers from contacts sent by frontend
    ...
    // Find which numbers are already registered users
